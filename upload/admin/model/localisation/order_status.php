...
class ModelLocalisationOrderStatus extends Model {
	...
	public function getOrderStatuses($data = array()) {
      	if ($data) {
			$sql = "SELECT * FROM {order_status} WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'";
			...
			if (!$order_status_data) {
				$query = $this->db->query("SELECT order_status_id, name FROM {order_status} WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY name");
