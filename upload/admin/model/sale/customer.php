...
class ModelSaleCustomer extends Model {
	...
	public function getCustomers($data = array()) {
		$sql = "SELECT *, CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group FROM {customer} c LEFT JOIN {customer_group_description} cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getHistories($customer_id, $start = 0, $limit = 10) { 
		...
		$query = $this->db->query("SELECT comment, date_added FROM {customer_history} WHERE customer_id = '" . (int)$customer_id . "' ORDER BY date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
	...
	public function addTransaction($customer_id, $description = '', $amount = '', $order_id = 0) {
		...
		if ($customer_info) { 
			$this->db->query("INSERT INTO {customer_transaction} SET customer_id = '" . (int)$customer_id . "', order_id = '" . (int)$order_id . "', description = '" . $this->db->escape($description) . "', amount = '" . (float)$amount . "', date_added = NOW()");
	...
	public function getTransactions($customer_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT * FROM {customer_transaction} WHERE customer_id = '" . (int)$customer_id . "' ORDER BY date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
	...
	public function addReward($customer_id, $description = '', $points = '', $order_id = 0) {
		...
		if ($customer_info) { 
			$this->db->query("INSERT INTO {customer_reward} SET customer_id = '" . (int)$customer_id . "', order_id = '" . (int)$order_id . "', points = '" . (int)$points . "', description = '" . $this->db->escape($description) . "', date_added = NOW()");
	...
	public function getRewards($customer_id, $start = 0, $limit = 10) {
		$query = $this->db->query("SELECT * FROM {customer_reward} WHERE customer_id = '" . (int)$customer_id . "' ORDER BY date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
