...
class ModelUpgrade extends Model {
	public function mysql() {
		...
		// Settings
		$query = $this->db->query("SELECT * FROM {setting} WHERE `store_id` = '0' ORDER BY `store_id` ASC");
		...
		if (empty($settings['config_voucher_min'])) {
			$this->db->query("INSERT INTO {setting} SET `value` = '1', `key` = 'config_voucher_min', `group` = 'config', `store_id` = 0");
		}
		
		if (empty($settings['config_voucher_max'])) {
			$this->db->query("INSERT INTO {setting} SET `value` = '1000', `key` = 'config_voucher_max', `group` = 'config', `store_id` = 0");
		}
		...
		if (in_array('name', $table_old_data[DB_PREFIX . 'customer_group'])) {
			...
			$customer_group_query = $this->db->query("SELECT * FROM {customer_group}");
			
			foreach ($customer_group_query->rows as $customer_group) {
				$language_query = $this->db->query("SELECT `language_id` FROM {language}");
			
				foreach ($language_query->rows as $language) {
					$this->db->query("REPLACE INTO {customer_group_description} SET `customer_group_id` = '" . (int)$customer_group['customer_group_id'] . "', `language_id` = '" . (int)$language['language_id'] . "', `name` = '" . $this->db->escape($customer_group['name']) . "'");
				}
			}
